...
def score_stock(row):
    ...
    try:
        ...
        main_wave = macd_gold and macd_above_zero and last["MA20"] > df["MA20"].iloc[-5] and breakout
        ...
        breakout_rate = min(99, max(50, score + (5 if breakout else -8)))
        second_wave_rate = min(99, max(50, score + (6 if n_pattern else -10)))
...
st.warning("全上市上櫃掃描會比較久，手機版可能需要數分鐘。建議先用前100檔測試。")
...
if st.button("🔥 開始掃描"):
    ...
    if results:
        ...
        if not top90.empty:
            for _, r in top90.head(30).iterrows():
                st.success(
                    f"🏅 {r['股號']}｜{r['股名']}｜{r['分數']}分\n\n"
                    f"🚀 主升段：{r['主升段']}\n\n"
                    f"🔥 N字第二波：{r['N字第二波']}\n\n"
